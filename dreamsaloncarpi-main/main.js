...
/* ================================================================
   GOOGLE MAPS – Caricamento condizionale (GDPR)
   Chiamata dall'inline onclick nel HTML oppure dopo consenso cookie
   ================================================================ */
function loadGoogleMap() {
  const placeholder = document.getElementById('map-placeholder');
  const iframeWrap = document.getElementById('map-iframe-wrap');
  const iframe = document.getElementById('map-iframe');
  if (placeholder && iframeWrap && iframe) {
...
/* ================================================================
   COOKIE CONSENT BANNER – GDPR / Garante Privacy IT
   ================================================================ */
(function () {
  const CONSENT_KEY = 'cookie_consent';
  const consent = localStorage.getItem(CONSENT_KEY);

  /* Se già accettato in sessione precedente: carica mappa automaticamente */
  if (consent === 'accepted') {
    window.addEventListener('DOMContentLoaded', loadGoogleMap);
  }

  /* Se già scelto (accettato o rifiutato): non mostrare il banner */
  if (consent) return;

  /* ---- Costruzione HTML del banner ---- */
  ...
  banner.innerHTML = `
    <div class="cookie-banner-inner">
      <div class="cookie-banner-text">
        ...
        <p>Questo sito usa cookie tecnici necessari al funzionamento e, previo consenso, cookie analitici (Google Analytics) e di terze parti (Google Maps). Per maggiori informazioni consulta la nostra <a href="privacy.html#cookie">Cookie Policy</a>.</p>
      </div>
      <div class="cookie-banner-actions">
        <button id="cookie-reject" class="cookie-btn cookie-btn-outline">Solo necessari</button>
        <button id="cookie-accept" class="cookie-btn cookie-btn-primary">Accetta tutto</button>
      </div>
    </div>
  `;
  ...
  document.getElementById('cookie-accept').addEventListener('click', function () {
    ...
    loadGoogleMap();
    /* Qui inserire gtag('consent', 'update', { analytics_storage: 'granted' }) dopo aggiunta GA4 */
  });
